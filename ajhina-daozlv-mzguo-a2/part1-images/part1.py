...
def groupMatrix(disMatrix,k):##1.2Clustering
   groups = []
...
def compare(filename1, filename2):##1.1 Image matching
...
def compareknn(filename1, filename2):##1.1 Image matching
   img1 = cv2.imread(filename1, cv2.IMREAD_GRAYSCALE)
   img2 = cv2.imread(filename2, cv2.IMREAD_GRAYSCALE)
   orb = cv2.ORB_create(nfeatures=1000)
...
def drawMatches(img1, kp1, img2, kp2, matches):##1.1 visualize
