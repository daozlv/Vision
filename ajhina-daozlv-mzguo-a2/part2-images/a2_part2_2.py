...
n = sys.argv[1]
if n!=4:
    sys.exit("Only n=4 is accepted unfortunately =(")

img1 = Image.open(sys.argv[2])
img0 = Image.open(sys.argv[3])
out_file = sys.argv[4]
img_arr = np.array(img0)

width1, height1 = img1.size
# width0, height0 = img0.size
img1_point_a = [int(i) for i in (sys.argv[5]).split(",")]
img0_point_a = [int(i) for i in (sys.argv[6]).split(",")]
img1_point_b = [int(i) for i in (sys.argv[7]).split(",")]
img0_point_b = [int(i) for i in (sys.argv[8]).split(",")]
img1_point_c = [int(i) for i in (sys.argv[9]).split(",")]
img0_point_c = [int(i) for i in (sys.argv[10]).split(",")]
img1_point_d = [int(i) for i in (sys.argv[11]).split(",")]
img0_point_d = [int(i) for i in (sys.argv[12]).split(",")]

T = get_transformation_matrix([img1_point_a, img1_point_b, img1_point_c, img1_point_d], [img0_point_a, img0_point_b, img0_point_c, img0_point_d])
